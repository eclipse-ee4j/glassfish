...
* xref:#using-the-glassfish-server-performance-tuner[Using the {productName} Performance Tuner]
* xref:#deployment-settings[Deployment Settings]
* xref:#logger-settings[Logger Settings]
* xref:#web-container-settings[Web Container Settings]
* xref:#ejb-container-settings[EJB Container Settings]
* xref:#java-message-service-settings[Java Message Service Settings]
* xref:#transaction-service-settings[Transaction Service Settings]
* xref:#http-service-settings[HTTP Service Settings]
* xref:#network-listener-settings[Network Listener Settings]
* xref:#transport-settings[Transport Settings]
* xref:#thread-pool-settings[Thread Pool Settings]
* xref:#orb-settings[ORB Settings]
* xref:#resource-settings[Resource Settings]
* xref:#load-balancer-settings[Load Balancer Settings]
...
* xref:#disable-auto-deployment[Disable Auto-Deployment]
* xref:#use-pre-compiled-javaserver-pages[Use Pre-compiled JavaServer Pages]
* xref:#disable-dynamic-application-reloading[Disable Dynamic Application Reloading]
...
further instructions. Alternatively, refer to "link:application-deployment-guide/deploying-applications.html#to-deploy-an-application-or-module-automatically[To
...
following the instructions in "link:administration-guide/webapps.html#administering-web-applications[Administering Web
Applications]" in {productName} Administration
Guide.

* xref:#session-properties-session-timeout[Session Properties: Session Timeout]
* xref:#manager-properties-reap-interval[Manager Properties: Reap Interval]
* xref:#disable-dynamic-jsp-reloading[Disable Dynamic JSP Reloading]
...
* xref:#overview-of-ejb-pooling-and-caching[Overview of EJB Pooling and Caching]
* xref:#tuning-the-ejb-pool[Tuning the EJB Pool]
* xref:#tuning-the-ejb-cache[Tuning the EJB Cache]
* xref:#pool-and-cache-settings-for-individual-ejb-components[Pool and Cache Settings for Individual EJB Components]
...
information on using JMS, see "link:administration-guide/jms.html#administering-the-java-message-service-jms[Administering the Java
...
* xref:#monitoring-the-transaction-service[Monitoring the Transaction Service]
* xref:#tuning-the-transaction-service[Tuning the Transaction Service]
...
follow the instructions in "link:administration-guide/transactions.html#administering-transactions[Administering
...
* xref:#monitoring-the-http-service[Monitoring the HTTP Service]
* xref:#http-service-access-logging[HTTP Service Access Logging]
...
Refer to "link:administration-guide/monitoring.html#administering-the-monitoring-service[Administering the Monitoring Service]" in
...
`asadmin` subcommands, see "link:administration-guide/monitoring.html#viewing-comprehensive-monitoring-data[Viewing Comprehensive
...
* xref:#dns-cache-information-dns[DNS Cache Information (dns)]
* xref:#file-cache-information-file-cache[File Cache Information (file-cache)]
* xref:#keep-alive-keep-alive[Keep Alive (keep-alive)]
* xref:#connection-queue[Connection Queue]
...
* xref:#cacheentries-currentcacheentries-maxcacheentries[CacheEntries (CurrentCacheEntries / MaxCacheEntries)]
* xref:#hitratio[HitRatio]
* xref:#caching-dns-entries[Caching DNS Entries]
* xref:#limit-dns-lookups-to-asynchronous[Limit DNS Lookups to Asynchronous]
* xref:#enabled-1[Enabled]
* xref:#namelookups[NameLookups]
* xref:#addrlookups[AddrLookups]
* xref:#lookupsinprogress[LookupsInProgress]
...
For information about tuning the file cache, see xref:#file-cache-settings[File Cache
Settings].
...
xref:#timeout[Timeout] for more information.
...
the instructions in "link:administration-guide/http_https.html#administering-http-network-listeners[Administering HTTP Network
...
* xref:#general-settings-1[General Settings]
* xref:#http-settings[HTTP Settings]
* xref:#file-cache-settings[File Cache Settings]
...
* xref:#max-connections[Max Connections]
* xref:#dns-lookup-enabled[DNS Lookup Enabled]
* xref:#timeout[Timeout]
* xref:#header-buffer-length[Header Buffer Length]
...
* xref:#max-file-count[Max File Count]
* xref:#max-age[Max Age]
...
See "link:administration-guide/http_https.html#administering-http-network-listeners-1[Administering HTTP Network Listeners]" in
{productName} Administration Guide for
...
"link:administration-guide/threadpools.html#administering-thread-pools[Administering Thread Pools]" in {productName} Administration Guide. If using the Administration Console
...
* xref:#max-thread-pool-size[Max Thread Pool Size]
* xref:#min-thread-pool-size[Min Thread Pool Size]
...
"link:administration-guide/orb.html#administering-the-object-request-broker-orb[Administering the Object Request Broker (ORB)]" in
{productName} Administration Guide. Also refer to
"link:ha-administration-guide/rmi-iiop.html#rmi-iiop-load-balancing-and-failover[RMI-IIOP Load Balancing and Failover]" in {productName} High Availability Administration Guide. You
...
* xref:#how-a-client-connects-to-the-orb[How a Client Connects to the ORB]
* xref:#monitoring-the-orb[Monitoring the ORB]
* xref:#tuning-the-orb[Tuning the ORB]
...
* xref:#connection-statistics[Connection Statistics]
* xref:#thread-pools[Thread Pools]
...
* xref:#tunable-orb-parameters[Tunable ORB Parameters]
* xref:#orb-thread-pool-parameters[ORB Thread Pool Parameters]
* xref:#client-orb-properties[Client ORB Properties]
* xref:#thread-pool-sizing[Thread Pool Sizing]
* xref:#examining-iiop-messages[Examining IIOP Messages]
...
"link:administration-guide/threadpools.html#administering-thread-pools[Administering Thread Pools]" in {productName} Administration Guide. If using the Administration
...
more information, see xref:#thread-pool-sizing[Thread Pool Sizing].
...
* xref:#controlling-connections-between-client-and-server-orb[Controlling Connections Between Client and Server ORB]
* xref:#limiting-the-maximum-number-of-client-connections[Limiting the Maximum Number of Client Connections]
* xref:#load-balancing[Load Balancing]
...
server instances in a cluster, "link:ha-administration-guide/rmi-iiop.html#rmi-iiop-load-balancing-and-failover[RMI-IIOP Load Balancing
...
* xref:#jdbc-connection-pool-settings[JDBC Connection Pool Settings]
* xref:#connector-connection-pool-settings[Connector Connection Pool Settings]
...
Refer to "link:administration-guide/jdbc.html#administering-database-connectivity[Administering Database Connectivity]" in
{productName} Administration Guide for more
information about managing JDBC connection pools.

The following topics are addressed here:

* xref:#monitoring-jdbc-connection-pools[Monitoring JDBC Connection Pools]
* xref:#tuning-jdbc-connection-pools[Tuning JDBC Connection Pools]
...
"link:administration-guide/monitoring.html#administering-the-monitoring-service[Administering the Monitoring Service]" in {productName} Administration Guide. If using the
...
* xref:#pool-size-settings[Pool Size Settings]
* xref:#timeout-settings[Timeout Settings]
* xref:#isolation-level-settings[Isolation Level Settings]
* xref:#connection-validation-settings[Connection Validation Settings]
...
section, xref:#tuning-jdbc-connection-pools[Tuning JDBC Connection Pools].
...
information on LAO, see xref:tuning-apps.adoc#configure-jdbc-resources-as-one-phase-commit-resources[Configure JDBC
...
* "link:ha-administration-guide/http-load-balancing.html#configuring-http-load-balancing[Configuring HTTP Load Balancing]" in {productName} High Availability Administration Guide
* "link:ha-administration-guide/rmi-iiop.html#rmi-iiop-load-balancing-and-failover[RMI-IIOP Load Balancing and Failover]" in {productName} High Availability Administration Guide
