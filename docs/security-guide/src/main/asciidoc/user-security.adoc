...
`create-jvm-options` subcommand, see the xref:reference-manual.adoc#create-jvm-options[{productName} Reference Manual].
====
...
[source]
----
keytool -importcert -alias "alias-name" -keystore domain-dir/config/cacerts.p12
+
+NOTE: The above examples show PKCS12 format (.p12) which is the recommended
+format since Java 9. For legacy JKS format compatibility, use `.jks` file extension.
-file cert-name
----
+
...
--
+
For example, to import a certificate named `oi.cer` for a {productName} domain in `/glassfish9/glassfish/domains/domain1`, using an
...
/glassfish9/glassfish/domains/domain1/config/cacerts.p12 -file oid.cer
----
...
+
+NOTE: The above examples show PKCS12 format (.p12) which is the recommended
+format since Java 9. For legacy JKS format compatibility, use `.jks` file extensions.
-file cert-name
----
+
where the variables are defined as follows:
...
--
+
For example, to import a certificate named `ovd.cer` for a {productName} domain in `/glassfish9/glassfish/domains/domain1`, using an
...
/glassfish9/glassfish/domains/domain1/config/cacerts.p12 -file ovd.cer
----
