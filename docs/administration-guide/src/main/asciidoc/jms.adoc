...
providers. The last section in this chapter, xref:#gbtvg[Using the
...
* xref:#gbtvg[Using the Generic Resource Adapter for JMS to Integrate
...
`type`::
  ...
  `EMBEDDED`, `LOCAL` and `REMOTE`. See xref:#giplw[About JMS Host
...
on administering physical destinations, see xref:#ablkj[Administering
JMS Physical Destinations].
...
underlying connector connection pool, See xref:connectors.adoc#gioti[To
Update a Connector Connection Pool].

To update a destination, use the `set` subcommand for the admin object
resource. See xref:connectors.adoc#giots[To Update an Administered
Object].
...
xref:overview.adoc#ghciy[Configuration Changes That Require Restart]. If
your server needs to be restarted, see xref:domains.adoc#ginqj[To Restart
a Domain].
...
resource, see xref:#giotd[To Create a Connection Factory or Destination
Resource].
...
* xref:#gkkvc[Configuring GenericJMSRA for Supported External JMS
Providers]
...
+
See xref:connectors.adoc#gioce[To Create a Connector Connection Pool].
5. Create a connector resource.
+
See xref:connectors.adoc#giogt[To Create a Connector Resource].
6. Create an administered object resource.
+
See xref:connectors.adoc#giolr[To Create an Administered Object].
...
* xref:#gkmee[Create a Resource Adapter Configuration for GenericJMSRA
...
* <<Limitations When Using GenericJMSRA with WebLogic JMS>>
* xref:#gktfi[Configuration Reference of GenericJMSRA Properties for
WebLogic JMS]
...
relevant for WebLogic JMS, see the xref:#gktfi[Configuration Reference
...
configuration as described in xref:#gkmee[Create a Resource Adapter
...
that was deployed in the xref:#gkmee[Create a Resource Adapter
...
for WebLogic JMS, see the xref:#gktfi[Configuration Reference of
...
configuration, as shown in xref:#gkkwq[Configuring an MDB to Receive
...
create these resources, see xref:connectors.adoc#gioce[To Create a
Connector Connection Pool] and xref:connectors.adoc#giogt[To Create a
Connector Resource].
====
...
are relevant for WebLogic JMS, see the xref:#gktfi[Configuration
...
WebLogic JMS, see the xref:#gktfi[Configuration Reference of
GenericJMSRA Properties for WebLogic JMS].
...
of properties, see the comprehensive table in xref:#gbtvu[GenericJMSRA
...
* xref:#gksni[Create a Resource Adapter Configuration for GenericJMSRA
to Work With WebSphere MQ]
* <<Deploy the GenericJMSRA Archive>>
* xref:#gkson[Create the Connection Factories and Administered Objects
in {productName}]
...
properties, see the comprehensive table in xref:#gbtvu[GenericJMSRA
Configuration Properties].
====
...
Using the example WebSphere MQ configuration in xref:#gksli[Configure
...
`asadmin` to create these resources, see xref:connectors.adoc#gioce[To
Create a Connector Connection Pool] and xref:connectors.adoc#giogt[To
...
configuration in xref:#gksli[Configure the WebSphere MQ Administered
Objects].
...
map to the example WebSphere MQ configuration in xref:#gksli[Configure
