...
"xref:security-guide.adoc#to-change-an-administration-password[To Change an Administration Password]" in {productName} Server Security Guide.
...
Passwords]" in {productName} Server Security Guide.
...
* <<To Prevent Shutdown When a User Logs Out on Windows>>
...
Domain Directory: C:\glassfish9\glassfish\domains\domain1
Configuration file for Windows Services Wrapper: C:\glassfish9\glassfish\domains\
domain1\bin\domain1Service.xml
The service can be controlled using the Windows Services Manager or you can use the
Windows Services Wrapper instead:
Start Command:  C:\glassfish9\glassfish\domains\domain1\bin\domain1Service.exe  start
Stop Command:   C:\glassfish9\glassfish\domains\domain1\bin\domain1Service.exe  stop
Uninstall Command:  C:\glassfish9\glassfish\domains\domain1\bin\domain1Service.exe
uninstall
Install Command:  C:\glassfish9\glassfish\domains\domain1\bin\domain1Service.exe
install
...
/export/glassfish9/glassfish/domains/domain1/PlatformServices.log
Command create-service executed successfully.
----
...
/glassfish9/glassfish/domains
Manifest file location on the system:/var/svc/manifest/application
/GlassFish/domain1_home_gfuser_glassfish-installations_glassfish9
...
* "https://www.oracle.com/pls/topic/lookup?ctx=E18752&id=SYSADV1hbrunlevels-25516[Managing Services (Overview)]" in System Administration Guide: Basic Administration
* "https://www.oracle.com/pls/topic/lookup?ctx=E18752&id=SYSADV1faauf[Managing Services (Tasks)]" in System Administration Guide: Basic Administration
...
On old distributions of Windows operating systems (before Windows 7) it was possible
to limit signal set accepted by the JVM using the `-Xrs` JVM option and that
effectively prevented killing of the JVM.

This does not work any more, even worse, the JVM even doesn't execute shutdown hooks
and behaves as if user would use the SIGKILL signal on Linux.

The problem is registered by several issues reported to OpenJDK:

* "https://bugs.openjdk.org/browse/JDK-8253445[OpenJDK Bug 8253445 - Java applications on Windows do not execute shutdown hooks on system reboot/shutdown or user log-off]"

To prevent killing of GlassFish instances you have two options, for both user needs special permissions:

* To create a service using the xref:reference-manual.adoc#create-service[`create-service`] command.
* To use Windows Scheduler to start the instance. We use this way to manage SSH Nodes too.
  See "https://learn.microsoft.com/en-us/powershell/module/scheduledtasks/new-scheduledtask[Microsoft PowerShell: New-ScheduledTask]"
...
Restored the domain (domain1) to /home/user1/glassfish9/glassfish/domains/domain1
...
domain  Location: /export/glassfish9/glassfish/domains/domain1
Log File: /export/glassfish9/glassfish/domains/domain1/logs/server.log
Admin Port: 4849
