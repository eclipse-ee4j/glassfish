...
* xref:#beagj[Using Servlets]
* xref:#beagu[Using JavaServer Pages]
* xref:#beaha[Creating and Managing Sessions]
* xref:#ggrgt[Using Comet]
* xref:#beaft[Advanced Web Application Features]
...
* xref:#beagm[Caching Servlet Results]
* xref:#beagr[About the Servlet Engine]
...
For information about JSP caching, see xref:#beagw[JSP Caching].

The following topics are addressed here:

* xref:#beagn[Caching Features]
* xref:#beago[Default Cache Configuration]
* xref:#beagp[Caching Example]
* xref:#beagq[The CacheKeyGenerator Interface]
...
See xref:#beagq[The CacheKeyGenerator Interface].
...
* xref:#beags[Instantiating and Removing Servlets]
* xref:#beagt[Request Handling]
...
* xref:#beagv[JSP Tag Libraries and Standard Portable Tags]
* xref:#beagw[JSP Caching]
* xref:#beagz[Options for Compiling JSP Files]
...
* xref:#gaxbq[Enabling JSP Caching]
* xref:#gaxcr[Caching Scope]
* xref:#beagx[The `cache` Tag]
* xref:#beagy[The `flush` Tag]

For more information about response caching as it pertains to servlets,
see xref:#beagm[Caching Servlet Results].
...
`request`, `session`, or `application`. See xref:#gaxcr[Caching Scope].
|===
...
* xref:#beahb[Configuring Sessions]
* xref:#beahf[Session Managers]
...
* xref:#beahc[HTTP Sessions, Cookies, and URL Rewriting]
* xref:#beahd[Coordinating Session Access]
* xref:#ghmih[Saving Sessions During Redeployment]
* xref:#gixeb[Logging Session Attributes]
* xref:#beahe[Distributed Sessions and Persistence]
...
entire web container, see xref:#beagc[Using the `default-web.xml` File]
...
`replicated` (see xref:#gdshu[The `replicated` Persistence Type]), the
...
session persistence type reverts to `memory`. See xref:#gdshu[The
...
* xref:#beahg[The `memory` Persistence Type], the default
* xref:#beahh[The `file` Persistence Type], which uses a file to store
session data
* xref:#gdshu[The `replicated` Persistence Type], which uses other
...
sessions to be distributed. For details, see xref:#beahe[Distributed
...
* xref:#ggrgy[Introduction to Comet]
* xref:#ghgxk[Grizzly Comet]
* xref:#ghgxn[Bayeux Protocol]
...
* xref:#ghgxk[Grizzly Comet] — Based on ARP, this includes a set of APIs
...
* xref:#ghgxn[Bayeux Protocol] — Often referred to as `Cometd`, it
...
* xref:#ghhaw[Enabling Comet]
* xref:#ghhap[To Configure the `web.xml` File]
* xref:#ghhtv[To Write, Deploy, and Run the Client]
...
* xref:#beafu[Internationalization Issues]
* xref:#gilyj[Virtual Server Properties]
* xref:#beagb[Class Loader Delegation]
* xref:#beagc[Using the `default-web.xml` File]
* xref:#beagd[Configuring Logging and Monitoring in the Web Container]
* xref:#beage[Configuring Idempotent URL Requests]
* xref:#beagi[Header Management]
* xref:#geqrh[Configuring Valves and Catalina Listeners]
* xref:#geqpl[Alternate Document Roots]
* xref:#geurd[Using a context.xml File]
* xref:#gezcx[Enabling WebDav]
* xref:#ghepd[Using SSI]
* xref:#ghepi[Using CGI]
...
* xref:#beafv[The Server's Default Locale]
* xref:#beafw[Servlet Character Encoding]
...
* xref:#beagf[Specifying an Idempotent URL]
* xref:#beagg[Characteristics of an Idempotent URL]
...
information, see xref:#gilyj[Virtual Server Properties].
...
xref:#gilyj[Virtual Server Properties]. For more information about the
...
see xref:#beagc[Using the `default-web.xml` File].
