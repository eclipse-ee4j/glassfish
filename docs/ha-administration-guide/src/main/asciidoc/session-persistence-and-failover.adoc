...
* xref:#abdla[Overview of Session Persistence and Failover]
* xref:#abdle[Enabling the High Availability Session Persistence Service]
* xref:#abdlp[Stateful Session Bean Failover]
...
* xref:#abdlb[Requirements]
* xref:#abdlc[Restrictions]
* xref:#gksoq[Scope]
...
* xref:#abdlf[To Enable Availability for a Cluster, Standalone Instance or Container]
* xref:#abdll[Configuring Availability for Individual Web Applications]
* xref:#gkwqu[Configuring Replication and Multi-Threaded Concurrent Access to `HttpSessions`]
* xref:#abdln[Using Single Sign-on with Session Failover]
* xref:#gkyyl[Using Coherence*Web for HTTP Session Persistence]
...
For more information, see xref:#abdlu[Configuring Availability for an Individual Bean].
...
checkpointed methods, as described in xref:#abdlw[Specifying Methods to
Be Checkpointed]
