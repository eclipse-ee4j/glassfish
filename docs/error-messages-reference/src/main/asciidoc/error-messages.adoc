...
xref:troubleshooting-guide.adoc#GSTSG[{productName} Server Troubleshooting Guide].
...
[[sthref250]]AS-WEB-UTIL-00028 The [\{0}] attribute is not consistent in context.xml for virtual servers ::
  Cause: As described in the message.
+
  Action: Resolve the situation described in the message, if necessary.
