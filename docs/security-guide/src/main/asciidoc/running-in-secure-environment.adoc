...
it is recommended to use audit logging to track any denied directory or
...
that are recommended for use to secure your production environment.
...
environments, it is recommended to create at least one other account in the asadmin group in
...
|Disable the Administration Console in production environments. a|
For production deployments, consider disabling the Administration Console to reduce the attack surface and prevent unauthorized access to administrative functions.

The Administration Console provides powerful management capabilities that should not be exposed in production environments where remote administration is not required. Web-based administrative interfaces are vulnerable to Cross-Site Request Forgery (CSRF) attacks where malicious links can execute administrative commands if clicked while an administrator is logged in.

To disable the Administration Console:
...
Use command-line administration tools (`asadmin`) for production management tasks instead.

See xref:administration-guide.adoc#to-disable-the-administration-console-loading[To Disable the Administration Console Loading] for detailed instructions.
|===
