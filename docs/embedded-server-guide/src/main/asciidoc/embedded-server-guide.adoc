...
* xref:#giidt[Introduction to Embedded {productName}]
* xref:#gkubb[Embedded {productName} File System]
* xref:#giijw[Including the {productName} Embedded Server API in Applications]
* xref:#gijhs[Testing Applications with the Maven Plug-in for Embedded {productName}]
* xref:#gjlde[Using the EJB 3.1 Embeddable API with Embedded {productName}]
* xref:#gjlfe[Changing Log Levels in Embedded {productName}]
* xref:#gksmw[Default Java Persistence Data Source for Embedded {productName}]
* xref:#gihxf[Restrictions for Embedded {productName}]
...
* With the Embedded Server API (see xref:#giijw[Including the {productName} Embedded Server API in Applications])
* With the Maven Plug-in (see xref:#gijhs[Testing Applications with the
Maven Plug-in for Embedded {productName}])
* With the EJB 3.1 Embeddable API (see xref:#gjlde[Using the EJB 3.1
...
* xref:#gikqf[Installation Root Directory]
* xref:#gikvf[Instance Root Directory]
* xref:#giiak[The `domain.xml` File]
...
xref:#gihyr[Creating and Configuring an Embedded {productName}].
====
...
* xref:#giide[Setting the Class Path]
* xref:#giiky[Creating, Starting, and Stopping Embedded {productName}]
* xref:#giigc[Deploying and Undeploying an Application in an Embedded
{productName}]
* xref:#gjldy[Running `asadmin` Commands Using the {productName}
Embedded Server API]
* xref:#gikrt[Sample Applications]
...
`glassfish-embedded-static-shell.jar`::
  ...
  explanation of as-install, see xref:#gikqf[Installation Root
  Directory].
...
* xref:#gihyr[Creating and Configuring an Embedded {productName}]
* xref:#gihyz[Running an Embedded {productName}]
...
|References an existing xref:#gikqf[Installation Root Directory], also called as-install
a|[source]
----
...
xref:#gikqf[Installation Root Directory] is automatically determined and
...
|===
|Purpose |Method |Default Value
|References an existing xref:#gikvf[Instance Root Directory], also
called domain-dir
a|
[source]
----
...
xref:#gihyr[Creating and Configuring an Embedded {productName}], you
can perform operations such as:

* xref:#gjkxx[Setting the Port of an Embedded {productName} From an Application]
* xref:#gihzg[Starting an Embedded {productName} From an Application]
* xref:#gihzy[Stopping an Embedded {productName} From an Application]
...
port. For more information, see xref:#giiak[The `domain.xml` File].
...
* xref:#gilrf[To Deploy an Application From an Archive File or a Directory]
* xref:#gilpm[Undeploying an Application]
* xref:#gjrcs[Creating a Scattered Archive]
* xref:#gkvgc[Creating a Scattered Enterprise Archive]
...
application's directory, see xref:#gjrcs[Creating a Scattered Archive].
...
Running link:reference-manual/asadmin.html#asadmin[`asadmin`] commands from an application enables
...
information, see xref:#gihyz[Running an Embedded {productName}].
====
...
For another way to change log levels, see xref:#gjlfe[Changing Log
Levels in Embedded {productName}].

[[gikrt]][[sample-applications]]
...
* xref:#gihus[To Set Up Your Maven Environment]
* xref:#giivt[To Build and Start an Application From Maven]
* xref:#gijly[To Stop Embedded {productName}]
* xref:#gijjn[To Redeploy an Application That Was Built and Started From Maven]
* xref:#gjkps[Maven Goals for Embedded {productName}]
...
API, see xref:#gjxff[Using Maven with the EJB 3.1 Embeddable API and
...
app::
  ...
  show example values. For details, see xref:#gjkps[Maven Goals for
  Embedded {productName}].
...
----
goal::
  The goal to use. See xref:#gjkps[Maven Goals for Embedded {productName}].
...
See xref:#gihzy[Stopping an Embedded {productName} From an
Application].
...
* xref:#gjkxo[`embedded-glassfish:run` Goal]
* xref:#gjkwy[`embedded-glassfish:start` Goal]
* xref:#gjkvc[`embedded-glassfish:deploy` Goal]
* xref:#gjkvg[`embedded-glassfish:undeploy` Goal]
* xref:#gjkyl[`embedded-glassfish:stop` Goal]
* xref:#gjkxq[`embedded-glassfish:admin` Goal]
...
|installRoot |None |(optional) The xref:#gikqf[Installation Root Directory].
...
* as-install``/domains/domain1`` if a nonembedded installation is referenced
 ...
 |(optional) The xref:#gikvf[Instance Root Directory]
...
|autoDelete |`false` a|
(optional) If `true`, deletes the contents of the xref:#gikvf[Instance
...
|installRoot |None |(optional) The xref:#gikqf[Installation Root
Directory].
...
However, you can use Maven with the POM file shown in xref:#gjxff[Using
...
* xref:#gjlcr[To Use the EJB 3.1 Embeddable API with Embedded {productName}]
* xref:#gksnv[EJB 3.1 Embeddable API Properties]
* xref:#gjxff[Using Maven with the EJB 3.1 Embeddable API and Embedded
{productName}]
...
+
See xref:#giide[Setting the Class Path] and Section 22.3.3 of the EJB
...
need to specify a JDBC resource. See xref:#gksmw[Default Java
...
4. To change the xref:#gikvf[Instance Root Directory], set the
...
+
The default xref:#gikvf[Instance Root Directory] location is
...
* as-install``/domains/domain1`` if a nonembedded installation is referenced
 ...
 |The xref:#gikvf[Instance Root Directory].
