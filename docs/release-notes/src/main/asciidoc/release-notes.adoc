...
* xref:#ghmvk["Revision History"]
* xref:#glcmf["What's New in the {productName} 7 Release?"]
* xref:#abpaj["Hardware and Software Requirements"]
* xref:#glcmm["Known Issues in {productName} 7"]
* xref:#ggqok["Restrictions and Deprecated Functionality"]
* xref:#BABDACBE["Documentation Errata"]
* xref:#giraq["Features Available Only in the Full Platform"]
* xref:#gipkz["Jakarta EE Standards Support"]
* xref:#abpha["How to Report Problems and Provide Feedback"]
* xref:#abphc["Additional Resources"]
...
see xref:#gipkz[Jakarta EE Standards Support].
...
* xref:#abpak[Required JDK Versions]
* xref:#ggppy[Required Disk Space]
* xref:#ggpoq[Required Free Ports]
* xref:#gkxlg[Message Queue Broker Requirements]
* xref:#gfgps[Paths and Environment Settings for the JDK Software]
...
Also be sure to see xref:#gfgps[Paths and Environment Settings for the
...
* xref:#gldjj[Use the JDK Binaries]
* xref:#gldgl[Set the `JAVA_HOME` Environment Variable]
* xref:#gldho[Set Other Environment Variables As Necessary]
...
* xref:#CHDIBHHF[JAXB and JAX-WS no longer part of Jakarta EE platform]
* xref:#CHDIBEAJ[Resource validation property is enabled in the JVM option for deployment]
* xref:#CHDBJDGF[Update Tool and pkg Command no longer part of {productName}]
* xref:#CHDJGJHD[Java DB has been replaced by Apache Derby]
...
* xref:#BABIDEHD[Upgrading to {productName} Is Not Necessary]
