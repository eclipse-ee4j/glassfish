...
In-place::
  An in-place upgrade means that the current installation itself is upgraded and replaces the older installation. {productName} doesn't support this way of upgrading directly and it requires several manual steps, including a significant downtime of services:

  1. Move the current installation into a different directory
  2. Perform a basic installation of {productName} {product-majorVersion} in the original location where the older product was installed.
  3. Copy the configuration from old installation to the new one.
  4. Start the new {productName} installation. Test it to make sure everything
  is working properly. If you find issues, copy the older installation over the new one to restore the old installation.
...
The {productName} Upgrade Tool is intended for performing an upgrade of configuration from any compatible older product version to configuration for {productName} {product-majorVersion}. 
...
[NOTE]
====
Upgrade Tool is the only tool provided by {productName} {product-majorVersion} that you can use when upgrading to {productName} {product-majorVersion} from older versions.
====
...
upgrade to {productName} {product-majorVersion} from an older version.
...
4. Start the upgraded {productName} {product-majorVersion} domain.
+
[source]
----
...
5. If you are upgrading a clustered configuration or a configuration in
