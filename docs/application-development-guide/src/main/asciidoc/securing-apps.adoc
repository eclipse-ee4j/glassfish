...
* xref:#beabh[Security Goals]
* xref:#beabi[{productName} Specific Security Features]
* xref:#beabj[Container Security]
* xref:#beacr[Roles, Principals, and Principal to Role Mapping]
* xref:#beabo[Realm Configuration]
* xref:#BACDEIHB[Jakarta EE Security API Support]
* xref:#beabt[JACC Support]
* xref:#beabu[Pluggable Audit Module Support]
* xref:#beabx[The `server.policy` File]
* xref:#beaca[Configuring Message Security for Web Services]
* xref:#beacm[Programmatic Login Using the ProgrammaticLogin Class]
* xref:#beacq[User Authentication for Single Sign-on]
* xref:#gizel[Adding Authentication Mechanisms to the Servlet Container]
...
* Message security; see xref:#beaca[Configuring Message Security for Web
Services]
* Single sign-on across all {productName} applications within a
single security domain; see xref:#beacq[User Authentication for Single
Sign-on]
* Programmatic login; see xref:#beacm[Programmatic Login Using the
...
* xref:#beabl[Declarative Security]
* xref:#beabk[Programmatic Security]
...
* xref:#beabm[Application Level Security]
* xref:#beabn[Component Level Security]
...
For more information, see xref:#beacr[Roles, Principals, and Principal
to Role Mapping].
...
* xref:#beabp[Supported Realms]
* xref:#beabq[How to Configure a Realm]
* xref:#beabr[How to Set a Realm for an Application or Module]
* xref:#beabs[Creating a Custom Realm]
...
For information about configuring realms, see xref:#beabq[How to
Configure a Realm].
...
* xref:#beabv[Configuring an Audit Module]
* xref:#beabw[The `AuditModule` Class]
...
* xref:#beaby[Default Permissions]
* xref:#gilzz[System Properties]
* xref:#beabz[Changing Permissions for an Application]
* xref:#gbyah[Enabling and Disabling the Security Manager]
...
* xref:#gbjxw[Message Security Providers]
* xref:#beacb[Message Security Responsibilities]
* xref:#beacf[Application-Specific Message Protection]
* xref:#beaci[Understanding and Running the Sample Application]
...
The example described in xref:#beaci[Understanding and Running the
...
settings, see xref:#beacf[Application-Specific Message Protection].
...
See xref:#gilzz[System Properties].
...
* xref:#beacc[Application Developer Responsibilities]
* xref:#beacd[Application Deployer Responsibilities]
* xref:#beace[System Administrator Responsibilities]
...
These security tasks are discussed in xref:#beacf[Application-Specific
Message Protection]. A sample application using message security is
discussed in xref:#beaci[Understanding and Running the Sample
Application].
...
* xref:#beacg[Using a Signature to Enable Message Protection for All
Methods]
* xref:#beach[Configuring Message Protection for a Specific Method Based
...
methods, leave the method-name element blank. xref:#beach[Configuring
...
xref:#beaci[Understanding and Running the Sample Application] to apply
...
app-dir`/xms-ejb/src/conf`, where app-dir is defined in xref:#beacj[To
...
xref:#beack[To Run the Sample Application].
...
steps. As in the example discussed in xref:#beacg[Using a Signature to
...
1.4 samples. If you have not installed these samples, see xref:#beacj[To
Set Up the Sample Application].
...
in xref:#beacj[To Set Up the Sample Application].
...
the application level, see xref:#beacf[Application-Specific Message
...
* xref:#beacj[To Set Up the Sample Application]
* xref:#beack[To Run the Sample Application]
...
* xref:#beacn[Programmatic Login Precautions]
* xref:#beaco[Granting Programmatic Login Permission]
* xref:#beacp[The `ProgrammaticLogin` Class]
...
xref:#beabx[The `server.policy` File]. This permission is not granted by
...
* xref:#BACCFGBF[The {productName} and JSR-375]
* xref:#gizfz[The {productName} and JSR 196]
* xref:#gizdx[Writing a Server Authentication Module]
* xref:#gizeb[Sample Server Authentication Module]
* xref:#gizfa[Compiling and Installing a Server Authentication Module]
* xref:#gizfe[Configuring a Server Authentication Module]
* xref:#gizfm[Binding a Server Authentication Module to Your
Application]
