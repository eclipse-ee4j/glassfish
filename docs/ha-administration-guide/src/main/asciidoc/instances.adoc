...
Centralized administration requires the  secure shell (SSH) to be set up. If
...
about setting up SSH, see xref:ssh-setup.adoc#enabling-centralized-administration-of-glassfish-server-instances[Enabling
...
communication, see the following section:

** xref:nodes.adoc#to-create-an-ssh-node[To Create an `SSH` Node]

* The user of the DAS can use SSH to log in to the host for the
...
running.
* The user of the DAS can use SSH to log in to the host for any
...
For more information, see xref:application-deployment-guide.adoc[{productName} Application Deployment Guide].
...
instance Location: /export/glassfish9/glassfish/nodes/sj02/pmd-i2
Log File: /export/glassfish9/glassfish/nodes/sj02/pmd-i2/logs/server.log
Admin Port: 24851
...
For more information, see the xref:application-deployment-guide.adoc[{productName} Application Deployment Guide].
...
Local administration does not require SSH to be set up. If
...
[NOTE]
====
Even if SSH is not set up, you can obtain information about
...
NODE NAME          TYPE    NODE HOST         INSTALL DIRECTORY   REFERENCED BY
localhost-domain1  CONFIG  localhost         /export/glassfish9
xk01               CONFIG  xk01.example.com  /export/glassfish9  kui-i1
Command list-nodes executed successfully.
----
...
instance Location: /export/glassfish9/glassfish/nodes/sj01/yml-i1
Log File: /export/glassfish9/glassfish/nodes/sj01/yml-i1/logs/server.log
Admin Port: 24849
Command start-local-instance executed successfully.
----
...
instance-name::
  The name of the instance that you are starting.
* If SSH is not set up, start the instance locally from the
...
[[gksfr]]
Example 5-17 Updating the Memory Configuration

This example updates changes the maximal heap size for the instance `pmd`.
...
`/export/glassfish9/glassfish/domains/domain1/myapp/myfile`.
