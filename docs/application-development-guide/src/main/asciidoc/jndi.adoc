...
* xref:#beans[Accessing the Naming Context]
* xref:#beanx[Configuring Resources]
* xref:#gcpge[Using a Custom `jndi.properties` File]
* xref:#beaoa[Mapping References]
...
* xref:#BGBJGGAD[Portable Global JNDI Names]
* xref:#gcjkd[{productName} V2 Vendor-Specific Global JNDI Names]
* xref:#gkndi[Disabling {productName} V2 JNDI Names]
* xref:#beanu[Accessing EJB Components Using the `CosNaming` Naming
Context]
* xref:#beanv[Accessing EJB Components in a Remote {productName}]
* xref:#beanw[Naming Environment for Lifecycle Modules]
...
xref:#gcjkd[{productName} V2 Vendor-Specific Global JNDI Names] and
xref:#gkndi[Disabling {productName} V2 JNDI Names].
...
descriptor file has the highest precedence. See xref:#beaoa[Mapping
References].
...
* xref:#beany[External JNDI Resources]
* xref:#beanz[Custom Resources]
* xref:#giyvw[Built-in Factories for Custom Resources]
* xref:#gkpdn[Using Application-Scoped Resources]
...
* xref:#giywi[JavaBeanFactory]
* xref:#giysn[PropertiesFactory]
* xref:#giytz[PrimitivesAndStringFactory]
* xref:#giywh[URLFactory]
