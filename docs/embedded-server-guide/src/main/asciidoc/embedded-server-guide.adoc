...
In `GlassFishProperties` and in a properties file, Embedded {productName} supports the same configuration properties as the `set` and `get` administration commands of the {productName} Server, with the following rules:

* If a property starts with `server.`, then it can be used directly
* All other properties need to be prefixed with `embedded-glassfish-config.` (e.g., `resources.jdbc-connection-pool...` needs to be defined as `embedded-glassfish-config.resources.jdbc-connection-pool...`). This is because these properties are internally merged with some other properties and it's necessary to distinguish them 
