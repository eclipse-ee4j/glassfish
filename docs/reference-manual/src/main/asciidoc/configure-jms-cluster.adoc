...
"xref:administration-guide.adoc#administering-the-java-message-service-jms[Administering the Java Message Service (JMS)]" in
...
`--clustertype`::
  ...
  "link:{mq-tech-over-url}/broker-clusters.html#GMTOV00028[Broker Clusters]"
...
--
  ...
  for each vendor, see "link:{mq-admin-guide-url}/persistence-services.html#GMADG00244[JDBC-Based Persistence]" in
...
of the xref:asadmin.adoc#asadmin[`asadmin`(1M)] command after adding the entry
