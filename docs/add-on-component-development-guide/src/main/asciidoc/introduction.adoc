...
* xref:#ghmje[{productName} Modular Architecture and Add-On
Components]
* xref:#ghmrf[OSGi Alliance Module Management Subsystem]
* xref:#ghmnq[Hundred-Kilobyte Kernel]
* xref:#ghmns[Overview of the Development Process for an Add-On
Component]
...
* xref:#ghpqc[Writing HK2 Components]
* xref:#ghokn[Extending the Administration Console]
* xref:#ghohx[Extending the `asadmin` Utility]
* xref:#ghojs[Adding Monitoring Capabilities]
* xref:#gkahs[Adding Configuration Data for a Component]
* xref:#ghojq[Adding Container Capabilities]
* xref:#gktld[Creating a Session Persistence Module]
* xref:#ghoiu[Packaging and Delivering an Add-On Component]
