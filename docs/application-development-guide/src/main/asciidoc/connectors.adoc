...
* xref:#beall[Connector Support in the {productName}]
* xref:#bealr[Advanced Connector Configuration Options]
* xref:#bealz[Inbound Communication Support]
* xref:#gizdm[Outbound Communication Support]
* xref:#beama[Configuring a Message Driven Bean to Use a Resource Adapter]
...
* xref:#beals[Thread Associations]
* xref:#bealt[Security Maps]
* xref:#giqgt[Work Security Maps]
* xref:#bealu[Overriding Configuration Properties]
* xref:#bealv[Testing a Connector Connection Pool]
* xref:#gizcv[Flushing a Connector Connection Pool]
* xref:#bealw[Handling Invalid Connections]
* xref:#bealx[Setting the Shutdown Timeout]
* xref:#gjjyy[Specifying the Class Loading Policy]
* xref:#bealy[Using Last Agent Optimization of Transactions]
* xref:#gizba[Disabling Pooling for a Connection]
* xref:#gizcs[Using Application-Scoped Connectors]
