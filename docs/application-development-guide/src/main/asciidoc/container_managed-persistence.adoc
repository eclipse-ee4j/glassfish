...
* xref:#glassfish-server-support-for-cmp[{productName} Support for CMP]
* xref:#cmp-mapping[CMP Mapping]
* xref:#automatic-schema-generation-for-cmp[Automatic Schema Generation for CMP]
* xref:#schema-capture[Schema Capture]
* xref:#configuring-the-cmp-resource[Configuring the CMP Resource]
* xref:#performance-related-features[Performance-Related Features]
* xref:#GSDVG00157[Configuring Queries for 1.1 Finders]
* xref:#cmp-restrictions-and-optimizations[CMP Restrictions and Optimizations]
...
** Support for commit options B and C for transactions.
   See link:transaction-service.html#commit-options[Commit Options].
...
* xref:#mapping-capabilities[Mapping Capabilities]
* xref:#the-mapping-deployment-descriptor-file[The Mapping Deployment Descriptor File]
* xref:#mapping-considerations[Mapping Considerations]
...
xref:#generation-options-for-cmp[Generation Options for CMP].
...
deploy the bean (see xref:#automatic-database-schema-capture[Automatic Database Schema Capture]).
You can manually generate the schema using the `capture-schema` utility
(xref:#using-the-capture-schema-utility[Using the `capture-schema` Utility]).
...
* xref:#join-tables-and-relationships[Join Tables and Relationships]
* xref:#automatic-primary-key-generation[Automatic Primary Key Generation]
* xref:#fixed-length-char-primary-keys[Fixed Length CHAR Primary Keys]
* xref:#managed-fields[Managed Fields]
* xref:#blob-support[BLOB Support]
* xref:#clob-support[CLOB Support]
...
xref:#supported-data-types-for-cmp[Supported Data Types for CMP].
...
See xref:#default-fetch-group-flags[Default Fetch Group Flags] and
"link:application-deployment-guide/dd-elements.html#fetched-with[fetched-with]" in {productName}
...
"link:administration-guide/jdbc.html#configuration-specifics-for-jdbc-drivers[Configuration Specifics for JDBC Drivers]" in
...
resource. See xref:#configuring-the-cmp-resource[Configuring the CMP Resource].
...
the data into the tables. See link:ejb.html#using-read-only-beans[Using Read-Only Beans].
...
xref:#version-column-consistency-checking[Version Column Consistency Checking].
====
...
`cmp-resource` element, see "link:application-deployment-guide/dd-elements.html#cmp-resource[cmp-resource]" in
...
|link:application-deployment-guide/dd-elements.html#create-tables-at-deploy[`create-tables-at-deploy`] |`false` |If `true`, causes
...
|link:application-deployment-guide/dd-elements.html#drop-tables-at-undeploy[`drop-tables-at-undeploy`] |`false` |If `true`, causes
...
|link:application-deployment-guide/dd-elements.html#database-vendor-name[`database-vendor-name`] |none a|
...
|link:application-deployment-guide/dd-elements.html#schema-generator-properties[`schema-generator-properties`] |none a|
...
"link:application-deployment-guide/dd-elements.html#schema-generator-properties[schema-generator-properties]" in {productName} Application Deployment Guide.
...
* xref:#automatic-database-schema-capture[Automatic Database Schema Capture]
* xref:#using-the-capture-schema-utility[Using the `capture-schema` Utility]
...
properly configured CMP resource. See xref:#configuring-the-cmp-resource[Configuring the CMP
Resource].
====
...
`glassfish-ejb-jar.xml` file. See "link:application-deployment-guide/dd-elements.html#cmp-resource[cmp-resource]" in
...
* xref:#version-column-consistency-checking[Version Column Consistency Checking]
* xref:#relationship-prefetching[Relationship Prefetching]
* xref:#read-only-beans[Read-Only Beans]
* xref:#default-fetch-group-flags[Default Fetch Group Flags]
...
file. See "link:application-deployment-guide/dd-elements.html#consistency[consistency]" in {productName} Application Deployment Guide.
...
performance.) See "link:application-deployment-guide/dd-elements.html#fetched-with[fetched-with]" in {productName} Application Deployment Guide.
...
"link:application-deployment-guide/dd-elements.html#prefetch-disabled[prefetch-disabled]" in {productName} Application Deployment Guide.
...
usage guidelines, see link:ejb.html#using-read-only-beans[Using Read-Only Beans].
...
For additional information, see "link:application-deployment-guide/dd-elements.html#level[level]" in {productName} Application Deployment Guide.
...
* xref:#about-jdoql-queries[About JDOQL Queries]
* xref:#query-filter-expression[Query Filter Expression]
* xref:#query-parameters[Query Parameters]
* xref:#query-variables[Query Variables]
* xref:#jdoql-examples[JDOQL Examples]
...
* xref:#disabling-order-by-validation[Disabling ORDER BY Validation]
* xref:#setting-the-heap-size-on-db2[Setting the Heap Size on DB2]
* xref:#eager-loading-of-field-state[Eager Loading of Field State]
* xref:#restrictions-on-remote-interfaces[Restrictions on Remote Interfaces]
* xref:#postgresql-case-insensitivity[PostgreSQL Case Insensitivity]
* xref:#no-support-for-lock-when-loaded-on-sybase[No Support for `lock-when-loaded` on Sybase]
* xref:#sybase-finder-limitation[Sybase Finder Limitation]
* xref:#date-and-time-fields[Date and Time Fields]
* xref:#set-recursive_triggers-to-false-on-mssql[Set `RECURSIVE_TRIGGERS` to `false` on MSSQL]
...
are used infrequently. See "link:application-deployment-guide/dd-elements.html#fetched-with[fetched-with]" in {productName} Application Deployment Guide.
...
Sybase databases. See "link:application-deployment-guide/dd-elements.html#consistency[consistency]" in {productName} Application Deployment Guide.
