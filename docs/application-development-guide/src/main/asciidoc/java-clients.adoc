...
* xref:#beaku[Introducing the Application Client Container]
* xref:#beakv[Developing Clients Using the ACC]
* xref:#gkusn[Developing Clients Without the ACC]
...
necessary; see xref:#gckgn[Using RMI/IIOP Over SSL].
...
clients by default. For more information, see xref:#gavmf[Using Java Web
Start].
...
* xref:#beakw[To Access an EJB Component From an Application Client]
* xref:#beakx[To Access a JMS Resource From an Application Client]
* xref:#gavmf[Using Java Web Start]
* xref:#gipkt[Using the Embeddable ACC]
* xref:#beaky[Running an Application Client Using the `appclient`
Script]
* xref:#beakz[Using the `package-appclient` Script]
* xref:#fvymy[The `client.policy` File]
* xref:#gckgn[Using RMI/IIOP Over SSL]
* xref:#ghbpc[Connecting to a Remote EJB Module Through a Firewall]
* xref:#gipkj[Specifying a Splash Screen]
* xref:#gjiec[Setting Login Retries]
* xref:#gjpjt[Using Libraries with Application Clients]
...
For more information, see xref:#beakz[Using the `package-appclient` Script]
...
xref:#beaky[Running an Application Client Using the `appclient` Script].
+
...
xref:#gavmf[Using Java Web Start] or xref:#beaky[Running an Application
...
client itself using the `asadmin get-client-stubs` command.
+
For more information, see xref:#beakz[Using the `package-appclient`
Script] and the link:reference-manual.html#GSRFM[{productName}
Reference Manual].
6. Run the application client.
+
See xref:#gavmf[Using Java Web Start] or xref:#beaky[Running an
...
* xref:#gavqb[Enabling and Disabling Java Web Start]
* xref:#gavmk[Downloading and Launching an Application Client]
* xref:#gavor[The Application Client URL]
* xref:#gcjlg[Signing JAR Files Used in Java Web Start]
* xref:#geqxg[Error Handling]
* xref:#geqwd[Vendor Icon, Splash Screen, and Text]
* xref:#gkmbs[Creating a Custom JNLP File]
...
client. See xref:#gavor[The Application Client URL].
...
* xref:#gcjmd[Automatically Signing JAR Files]
* xref:#gcjlr[Using the `jar-signing-alias` Deployment Property]
...
or a combination by using a custom JNLP file; see xref:#gkmbs[Creating a
Custom JNLP File].
...
* xref:#gkmco[Specifying the JNLP File in the Deployment Descriptor]
* xref:#gkmbq[Referring to JAR Files from the JNLP File]
* xref:#gkmbt[Referring to Other JNLP Files]
* xref:#gkmbr[Combining Custom and Automatically Generated Content]
...
properties as JVM options. See xref:#beaky[Running an Application Client
...
entry at development time. See xref:#beaky[Running an Application Client
Using the `appclient` Script].
...
+
See xref:#gipkt[Using the Embeddable ACC].
...
* xref:#gkusp[To access an EJB component from a stand-alone client]
* xref:#gkutw[To access an EJB component from a server-side module]
* xref:#gkuqa[To access a JMS resource from a stand-alone client]

For information about using the ACC, see xref:#beakv[Developing Clients
Using the ACC].
...
`package-appclient` script; see xref:#beakz[Using the
