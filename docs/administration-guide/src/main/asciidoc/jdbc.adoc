...
administration procedures, see xref:#ggndp[Administering JDBC
Resources].
...
JDBC resource that points to the connection pool. See xref:#ggnfv[To
Create a JDBC Connection Pool] and xref:#ggnda[To Create a JDBC
Resource]. The final step is to integrate the JDBC driver into an
administrative domain as described in xref:#ghatb[Integrating the JDBC
Driver].
...
* xref:#gkudf[Enabling the `jdbc/__default` Resource in a Clustered
Environment]
* <<Integrating the JDBC Driver>>
...
xref:overview.adoc#ghciy[Configuration Changes That Require Restart].
...
(the default is `false`). See xref:#ggnfv[To Create a JDBC Connection
...
pool. For instructions, see xref:#ggnfv[To Create a JDBC Connection
Pool].
...
* xref:#beana[DataDirect Type 4 Driver for Microsoft SQL Server
Database]
