...
* xref:#abfar[Jakarta EE Platform Overview]
* xref:#abfay[{productName} Components]
* xref:#abfca[Configuration Roadmap for High Availability of {productName}]
...
* xref:#abfaz[Server Instances]
* xref:#abfba[Administrative Domains]
* xref:#abfbc[Clusters]
* xref:#abfbe[Named Configurations]
* xref:#abfbf[HTTP Load Balancer Plug-in]
* xref:#abfbh[IIOP Load Balancing in a Cluster]
* xref:#abfbi[Message Queue and JMS Resources]
