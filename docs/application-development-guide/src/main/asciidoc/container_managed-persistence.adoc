...
* xref:#beajk[{productName} Support for CMP]
* xref:#beajl[CMP Mapping]
* xref:#beajv[Automatic Schema Generation for CMP]
* xref:#beajy[Schema Capture]
* xref:#beakb[Configuring the CMP Resource]
* xref:#beaki[Performance-Related Features]
* xref:#beakc[Configuring Queries for 1.1 Finders]
* xref:#beakm[CMP Restrictions and Optimizations]
...
** CMP runtime support. See xref:#beakb[Configuring the CMP Resource].
...
+
For details, see xref:#beaki[Performance-Related Features].
...
* xref:#beajm[Mapping Capabilities]
* xref:#beajn[The Mapping Deployment Descriptor File]
* xref:#beajo[Mapping Considerations]
...
xref:#beajx[Generation Options for CMP].
...
deploy the bean (see xref:#beajz[Automatic Database Schema Capture]).
You can manually generate the schema using the `capture-schema` utility
(xref:#beaka[Using the `capture-schema` Utility]).
...
* xref:#beajp[Join Tables and Relationships]
* xref:#beajq[Automatic Primary Key Generation]
* xref:#beajr[Fixed Length CHAR Primary Keys]
* xref:#beajs[Managed Fields]
* xref:#beajt[BLOB Support]
* xref:#beaju[CLOB Support]
...
xref:#beajw[Supported Data Types for CMP].
...
See xref:#gemln[Default Fetch Group Flags] and
...
resource. See xref:#beakb[Configuring the CMP Resource].
...
xref:#beakj[Version Column Consistency Checking].
====
...
* xref:#beajz[Automatic Database Schema Capture]
* xref:#beaka[Using the `capture-schema` Utility]
...
properly configured CMP resource. See xref:#beakb[Configuring the CMP
Resource].
====
...
* xref:#beakj[Version Column Consistency Checking]
* xref:#beakk[Relationship Prefetching]
* xref:#beakl[Read-Only Beans]
* xref:#gemln[Default Fetch Group Flags]
...
* xref:#ganjq[About JDOQL Queries]
* xref:#gankm[Query Filter Expression]
* xref:#ganjt[Query Parameters]
* xref:#ganky[Query Variables]
* xref:#ganla[JDOQL Examples]
...
* xref:#gdtfj[Disabling ORDER BY Validation]
* xref:#geprt[Setting the Heap Size on DB2]
* xref:#beakn[Eager Loading of Field State]
* xref:#beako[Restrictions on Remote Interfaces]
* xref:#gcnto[PostgreSQL Case Insensitivity]
* xref:#beakr[No Support for `lock-when-loaded` on Sybase]
* xref:#beakp[Sybase Finder Limitation]
* xref:#beakq[Date and Time Fields]
* xref:#beaks[Set `RECURSIVE_TRIGGERS` to `false` on MSSQL]
* xref:#gbhbr[MySQL Database Restrictions]
