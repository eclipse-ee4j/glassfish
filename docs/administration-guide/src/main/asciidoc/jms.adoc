...
Changing the Admin User Password for an Embedded or Local Broker::
  For security reasons, for new installations {productName} automatically
  generates the password for the Message Queue `admin` user when using
  Embedded or Local JMS hosts. To set a different password, follow these
  steps.

  You can retrieve the generated password in either of the following
  ways:

  * Read the `admin-password` attribute of the JMS host in
    domain-dir``/config/domain.xml``.
  * When the server is running, use the
    xref:reference-manual.adoc#get[`get`] subcommand to read the JMS host
    password:
+
[source]
----
asadmin> get configs.config.config-name.jms-service.jms-host.jms-host-name.admin-password
----
  ...
  [[CHDCJEFG]]
  . Update the JMS host configuration with the same custom value. You can
    store the custom value in a password alias and reference that alias
    from the JMS host configuration:
+
[source]
----
asadmin> set configs.config.server-config.jms-service.jms-host.default_JMS_host.admin-password='${ALIAS=jms-password}'
----
+
  For details about password aliases, see xref:security-guide.adoc#to-create-a-password-alias[To Create a Password Alias].
