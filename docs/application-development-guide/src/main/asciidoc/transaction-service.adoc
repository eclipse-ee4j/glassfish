...
* xref:#giybi[Handling Transactions with Databases]
* xref:#beaje[Handling Transactions with Enterprise Beans]
* xref:#gknrh[Handling Transactions with the Java Message Service]
* xref:#gaxit[The Transaction Manager, the Transaction Synchronization
...
* xref:#beamv[Using JDBC Transaction Isolation Levels]
* xref:#beamu[Using Non-Transactional Connections]
...
* xref:#beajf[Flat Transactions]
* xref:#beajg[Global and Local Transactions]
* xref:#beajh[Commit Options]
* xref:#beaht[Bean-Level Container-Managed Transaction Timeouts]
...
more information, see xref:#gaxit[The Transaction Manager, the
...
* xref:#beaoq[Transactions and Non-Persistent Messages]
* xref:#gdyya[Using the ConfigurableTransactionSupport Interface]
