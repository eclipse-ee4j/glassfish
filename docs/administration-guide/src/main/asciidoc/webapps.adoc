...
* xref:#gjpat[To Enable SSL Between the `mod_jk` Load Balancer and the
Browser]
* xref:#gjpan[To Enable SSL Between the `mod_jk` Load Balancer and
{productName}]
...
1. On each of the instances, perform the steps in xref:#gixqw[To Enable
`mod_jk`].
+
...
for requests, the web container must be started. Normally, this is
achieved by deploying a web application.
3. Follow instructions from xref:http_https.adoc#ggnbj[To Configure an
