...
* xref:#gjwpd[Binary-Compatible Releases For {productName} 7]
* xref:#gkric[New Default Installation Directory]
* xref:#gklhr[Changes to Group Management Service Settings]
* xref:#geyyk[Application Client Interoperability]
* xref:#gktad[Node Agent Support]
* xref:#gkrjm[HADB and `hadbm` Command Support]
* xref:#abmbc[Command Line Interface: The `asadmin` Command]
* xref:#gflnv[Applications That Use Java DB]
* xref:#gjizi[Applications That Use Persistence]
* xref:#gjiqj[HTTP Service to Network Service Changes]
* xref:#gktkt[NSS Cryptographic Token Support]
...
* xref:#abmbd[Deprecated `asadmin` Subcommands]
* xref:#abmbf[Deprecated, Unsupported, and Obsolete Options]
...
* xref:#gipsa[Changes to Dotted Names]
* xref:#giprg[Changes to `asadmin` Subcommands]
* xref:#gipfo[Remapping of HTTP Service Attributes and Properties]
* xref:#gipcg[New Network Service Elements and Attributes]
...
configuration, link:reference-manual/asadmin.html#asadmin[`asadmin`] subcommands are changed as
follows:
...
elements, see xref:#gipfo[Remapping of HTTP Service Attributes and
Properties].
