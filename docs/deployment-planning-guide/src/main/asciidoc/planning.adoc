...
* xref:#abfcc[Establishing Performance Goals]
* xref:#abfcp[Planning the Network Configuration]
* xref:#abfcy[Planning for Availability]
* xref:#abfdg[Design Decisions]
* xref:#abfdn[Planning Message Queue Broker Deployment]
...
* xref:#abfcd[Estimating Throughput]
* xref:#abfce[Estimating Load on {productName} Instances]
* xref:#abfcq[Estimating Bandwidth Requirements]
* xref:#abfct[Estimating Peak Load]
...
* xref:#abfcf[Maximum Number of Concurrent Users]
* xref:#abfcg[Think Time]
* xref:#abfch[Average Response Time]
* xref:#abfcj[Requests Per Minute]
...
* xref:#gkvdm[Setting Up Traffic Separation]
* xref:#abfcq[Estimating Bandwidth Requirements]
* xref:#abfcr[Calculating Bandwidth Required]
* xref:#abfct[Estimating Peak Load]
* xref:#abfcw[Choosing Network Cards]
* xref:#abfdc[Identifying Failure Classes]
...
To plan for traffic on both networks, see xref:#abfcq[Estimating
Bandwidth Requirements]. For external networks, follow the guidelines in
xref:#abfcr[Calculating Bandwidth Required] and xref:#abfct[Estimating
...
Based on the calculations made in xref:#abfcc[Establishing Performance
...
* xref:#abfcz[Rightsizing Availability]
* xref:#abfda[Using Clusters to Improve Availability]
* xref:#abfdb[Adding Redundancy to the System]
...
* xref:#abfdh[Designing for Peak or Steady State Load]
* xref:#abfdi[System Sizing]
* xref:#gkvcu[Sizing the Administration Thread Pool]
...
factors explained in xref:#abfce[Estimating Load on {productName}
...
* xref:#abfdo[Multi-Broker Clusters]
* xref:#abfdq[Configuring {productName} to Use Message Queue Brokers]
* xref:#abfdx[Example Deployment Scenarios]
