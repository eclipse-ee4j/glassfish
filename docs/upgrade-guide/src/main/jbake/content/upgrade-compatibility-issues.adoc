...
earlier product releases with which Oracle GlassFish Server 7 is
binary-compatible. When you upgrade to GlassFish Server 7, you must
address these issues.

The following topics are addressed here:

* link:#gjwpd[Binary-Compatible Releases For GlassFish Server 7]
...
GlassFish Server Open Source Edition 7 is binary-compatible with the
...
Server Open Source Edition 7 except for the compatibility issues that
...
GlassFish Server 7 and the Enterprise Server v3 releases do not affect
...
Server 7 in order to run the client. Use the
...
Server 7 to upgrade the GlassFish Server system files. This utility
...
and implementation of the RMI-IIOP protocol. GlassFish Server 7 does
...
Instead of HADB, GlassFish Server 7 supports high availability
...
Server 7. If you run the `asadmin start-database` command and
...
will work in GlassFish Server 7. The container creates an
...
When the application is loaded, GlassFish Server 7 will translate the
...
Eclipse wiki]
to do this. This tool is not provided with GlassFish Server 7,
...
The dotted name hierarchy for the GlassFish Server 7 Network Service
...
for Sun GlassFish Enterprise Server v3 and GlassFish Server 7. Note
...
----
* Command for GlassFish Server 7:
+
[source]
----
