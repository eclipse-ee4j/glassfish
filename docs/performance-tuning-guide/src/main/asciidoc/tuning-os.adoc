...
* xref:#abeis[Server Scaling]
* xref:#gfpzp[Solaris 10 Platform-Specific Tuning Information]
* xref:#abeix[Tuning for the Solaris OS]
* xref:#abeje[Tuning for Solaris on x86]
* xref:#abeji[Tuning for Linux platforms]
* xref:#gfpzh[Tuning UltraSPARC CMT-Based Systems]
...
* xref:#abeit[Processors]
* xref:#abeiu[Memory]
* xref:#abeiv[Disk Space]
* xref:#abeiw[Networking]
* xref:#glglf[UDP Buffer Sizes]
...
UDP buffer size as explained in xref:#glglz[To Set the UDP Buffer Size
on Linux Systems].
====
...
* xref:#abeiy[Tuning Parameters]
* xref:#abeja[File Descriptor Setting]
...
|512
|8192
|Size of connection hash table. See xref:#abeiz[Sizing the Connection Hash Table].
|===
...
* xref:#abejg[File Descriptors]
* xref:#abejh[IP Stack Settings]
...
* xref:#gkvjl[Startup Files]
* xref:#abejj[File Descriptors]
* xref:#abejk[Virtual Memory]
* xref:#abejl[Network Interface]
* xref:#abejm[Disk I/O Settings]
* xref:#abejn[TCP/IP Settings]
