...
* xref:#beaar[Installing and Preparing the Server for Development]
* xref:#beaas[High Availability Features]
* xref:#beaat[Development Tools]
* xref:#beabf[Sample Applications]
...
For more information, see "link:administration-guide/jvm.html#ablwj[
...
* xref:#beaau[The `asadmin` Command]
* xref:#beaav[The Administration Console]

The following development tools are provided with the {productName}
or downloadable from Oracle:

* xref:#beaba[The Migration Tool]
* xref:#beaaw[The NetBeans IDE]

The following third-party tools might also be useful:

* xref:#beabb[Debugging Tools]
* xref:#beabc[Profiling Tools]
