...
* xref:#abdbe[Overview]
* xref:#gknqo[`InitialContext` Load Balancing]
* xref:#gknpv[Per-Request Load Balancing (PRLB)]
...
* xref:#abdbf[General Requirements for Configuring Load Balancing]
* xref:#gknpk[Load Balancing Models]
...
in the Application Client Container (ACC). See xref:#abdbd[Enabling
...
* xref:#gknpn[`InitialContext` Summary]
* xref:#abdbg[`InitialContext` Algorithm]
* xref:#abdbd[Enabling RMI-IIOP Hardware Load Balancing and Failover]
...
* xref:#gksgt[PRLB Summary]
* xref:#gksgy[Enabling Per-Request Load Balancing]
