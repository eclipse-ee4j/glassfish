...
* xref:#CHDJJAEI[Overview of Oracle TopLink]
* xref:#CHDHDABJ[Using Oracle TopLink in {productName}]
* xref:#gbwmj[Specifying the Database for an Application]
* xref:#gdkwf[Specifying the Persistence Provider for an Application]
* xref:#gescc[Primary Key Generation Defaults]
* xref:#gbwlh[Automatic Schema Generation]
* xref:#gbxjh[Restrictions and Optimizations]
...
in The Jakarta EE Tutorial.
3. Create the `persistence.xml` file. See xref:#gbwmj[Specifying the
...
`Persistence.createEMF()`, see xref:#gdkwf[Specifying the Persistence
...
* xref:#gcjgl[Annotations]
* xref:#gbwmk[Generation Options]
...
database. See xref:#gbwmj[Specifying the Database for an Application].
====
...
* xref:#giqbi[Oracle Database Enhancements]
* xref:#ghdtr[Extended Persistence Context]
* xref:#gezxw[Using @OrderBy with a Shared Session Cache]
* xref:#gdwqi[Using BLOB or CLOB Types with the Inet Oraxo JDBC Driver]
* xref:#geqvh[Database Case Sensitivity]
* xref:#gbxjg[Sybase Finder Limitation]
* xref:#gbxjp[MySQL Database Restrictions]
...
For more information about the `eclipselink.target-database` property,
see xref:#gbwmj[Specifying the Database for an Application].
