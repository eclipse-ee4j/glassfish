...
`asadmin generate-jvm-report` command, see the xref:reference-manual.adoc#generate-jvm-report[{productName} Reference Manual].
...
information about the `appclient` script, see the xref:reference-manual.adoc#appclient[{productName} Reference Manual].
