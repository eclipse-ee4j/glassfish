...
xref:#gjxkx[Supported Releases for Upgrade to {productName} 7].

The following topics are addressed here:

* xref:#gaejc[Upgrade Overview]
* xref:#abmbr[Performing a Side-By-Side Upgrade With Upgrade Tool]
* xref:#gkthu[Performing an In-Place Upgrade With the Update Center
Tools]
* xref:#gktnh[Upgrading Installations That Use NSS Cryptographic Tokens]
* xref:#gfybw[Upgrading Clusters and Node Agent Configurations]
* xref:#gkrfh[Correcting Potential Upgrade Problems]
...
* xref:#gkxum[Upgrade Paths]
* xref:#gdkiw[Upgrade Terminology]
* xref:#gktcb[Summary of Upgrade Tools and Procedures]
* xref:#gjxkx[Supported Releases for Upgrade to {productName} 7]
* xref:#gkufi[Upgrading From Version 8.x or Older Product Releases]
* xref:#gkxrp[Upgrading {productName} Inside a Closed Network]
...
For a more detailed overview, see xref:#gktcb[Summary of Upgrade Tools
and Procedures].
...
* xref:#gktdx[Summary of Tools for Performing an Upgrade]
* xref:#gktdz[Summary of Procedure for Upgrading With Upgrade Tool]
* xref:#gktcl[Summary of Procedure for Upgrading With Update Tool]
* xref:#gkuef[Summary of Procedure for Upgrading With the Software Update Notifier]
* xref:#gktnb[Summary of Procedure for Upgrading With the `pkg` Utility]
...
* xref:#gktcz[Upgrade Tool]
* xref:#gktha[Update Tool and the `pkg` Utility]
* xref:#gkuff[Software Update Notifier]
...
xref:#gdkix[Upgrade Tool Functionality].
...
See xref:#gktdz[Summary of Procedure for Upgrading With Upgrade Tool]
...
must use the Upgrade Tool, described in xref:#gktcz[Upgrade Tool].
====

See xref:#gktcl[Summary of Procedure for Upgrading With Update Tool] for
...
See xref:#gkuef[Summary of Procedure for Upgrading With the Software
...
This procedure is described in more detail in xref:#abmbr[Performing a
...
This procedure is described in more detail in xref:#gktjf[To Upgrade
...
This procedure is described in more detail in xref:#gkuhu[To Upgrade
...
This procedure is described in more detail in xref:#gktks[To Upgrade
...
in xref:#gjxkx[Supported Releases for Upgrade to {productName} 7],
...
* xref:#gktgx[Upgrade Tool Summary]
* xref:#gdkix[Upgrade Tool Functionality]
* xref:#gktjn[To Upgrade From the Command Line Using Upgrade Tool]
* xref:#gaejn[To Upgrade Using the Upgrade Tool Wizard]
...
* xref:#gebrv[Migration of Deployed Applications]
* xref:#gebqm[Upgrade of Clusters]
* xref:#gebvn[Upgrade Verification]
...
clusters. This procedure is explained in xref:#gfybw[Upgrading Clusters
...
See xref:#gjxkx[Supported Releases for Upgrade to {productName} 7] for a list of supported releases.
...
Upgrade Tool starts in GUI mode, which is described in xref:#gaejn[To
...
authentication, follow the procedure in xref:#gktnh[Upgrading
...
Upgrade Tool, as described in xref:#abmbr[Performing a Side-By-Side
Upgrade With Upgrade Tool].
====

The following topics are addressed here:

* xref:#gkthx[Update Center Tool Procedures]
* xref:#gktjf[To Upgrade Using the Update Tool GUI]
* xref:#gkuhu[To Upgrade Using the Software Update Notifier]
* xref:#gktks[To Upgrade From the Command Line Using the `pkg` Utility]
...
* xref:#gktnq[To Prepare for the Upgrade]
* xref:#gktlz[To Perform Post-Upgrade Configuration]
* xref:#gktlp[To Upgrade PKCS#11 Hardware Tokens]
...
in xref:#gktnq[To Prepare for the Upgrade].
...
As part of the xref:#gktnq[To Prepare for the Upgrade] procedure, the
...
* xref:#gkyda[Overview of Cluster and Node Agent Upgrade Procedures]
* xref:#gktle[To Correct the Configuration of a Node After an Upgrade]
* xref:#gktkx[To Re-Create a Cluster]
...
described in xref:#abmbr[Performing a Side-By-Side Upgrade With Upgrade Tool].
...
This procedure is described in xref:#gktle[To Correct the Configuration
of a Node After an Upgrade].

4. Re-create the clusters and server instances on each {productName} 7 node host.
This procedure is described in xref:#gktkx[To Re-Create a Cluster].
...
re-create the instances as explained in xref:#gktkx[To Re-Create a
Cluster].
...
information, see xref:#abmbr[Performing a Side-By-Side Upgrade With Upgrade Tool].
...
* xref:#gkrgh[Cluster Profile Security Setting]
* xref:#gkrib[Cluster Profile Upgrade on Windows]
* xref:#gkyho[`asupgrade` Fails Without Internet Connection]
