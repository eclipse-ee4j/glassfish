...
created by the
xref:export-sync-bundle.adoc#export-sync-bundle[`export-sync-bundle`(1)]
subcommand.
...
`--port` option of the xref:asadmin.adoc#asadmin[`asadmin`(1M)]
utility.
...
`/export/glassfish9/glassfish/domains/domain1/sync/ymlcluster-sync-bundle.zip`.
