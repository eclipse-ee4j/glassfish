...
To enable virtual threads for an HTTP thread pool, set the thread pool's `virtual` option to `true`:
...
To revert to platform threads, set the value to empty or `false`:

[source]
----
asadmin> set server.thread-pools.thread-pool.http-thread-pool.virtual=
Command set executed successfully
----
