...
* xref:#giyck[Statements]
* xref:#giyeq[Connections]
* xref:#giyde[Connection Wrapping]
* xref:#gavro[Allowing Non-Component Callers]
* xref:#giydx[Using Application-Scoped Resources]
* xref:#geqvg[Restrictions and Optimizations]
...
* xref:#giyfu[Using an Initialization Statement]
* xref:#ghqrx[Setting a Statement Timeout]
* xref:#gktbf[Statement Leak Detection and Leaked Statement Reclamation]
* xref:#giyci[Statement Caching]
* xref:#giygg[Statement Tracing]
...
* xref:#gktas[Transparent Pool Reconfiguration]
* xref:#giygl[Disabling Pooling]
* xref:#giydr[Associating Connections with Threads]
* xref:#giyfg[Custom Connection Validation]
* xref:#beams[Sharing Connections]
* xref:#gezfh[Marking Bad Connections]
* xref:#gipzl[Handling Invalid Connections]
...
* xref:#ghqxi[Wrapping Connections]
* xref:#beamt[Obtaining a Physical Connection From a Wrapped Connection]
* xref:#ggrum[Using the `Connection.unwrap()` Method]
