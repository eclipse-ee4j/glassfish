...
prev=upgrading-legacy-installation.html
next=appendix.html
~~~~~~
...
== 2 {productName} Upgrade Compatibility Issues
...
[source]
----
user-home-directory/glassfish9
----
Windows systems::
[source]
----
...
The Jakarta EE {jakartaee} platform specification imposes stricter requirements than
...
Jakarta EE `library-directory` mechanism. Deployed Java EE 5 applications
...
conform to Jakarta EE {jakartaee} requirements.
...
xref:upgrading-legacy-installation.adoc#correcting-cluster-config[Correcting Cluster configuration] for more information.
...
subcommand is deprecated and will be removed in {productName} 7.1.0.
