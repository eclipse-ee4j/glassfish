...
* xref:#gjfqp[About {productName} Clusters]
* xref:#gjfnl[Group Management Service]
* xref:#gkqdy[Creating, Listing, and Deleting Clusters]
...
* xref:#CHDFEGAG[Protocols and Transports for GMS]
* xref:#gjfpd[GMS Configuration Settings]
* xref:#gjfog[Dotted Names for GMS Settings]
* xref:#gkoac[To Preconfigure Nondefault GMS Configuration Settings]
* xref:#gkqqo[To Change GMS Settings After Cluster Creation]
* xref:#gklhl[To Check the Health of Instances in a Cluster]
* xref:#gklhd[To Validate That Multicast Transport Is Available for a Cluster]
* xref:#CHDGAIBJ[Discovering a Cluster When Multicast Transport Is Unavailable]
* xref:#gjdlw[Using the Multi-Homing Feature With GMS]
...
information, see xref:#CHDGAIBJ[Discovering a Cluster When Multicast
...
For more information about these settings, see xref:#gkqdm[To Create a Cluster].
...
xref:#gjfog[Dotted Names for GMS Settings]. For the steps to specify
these settings, see xref:#gkoac[To Preconfigure Nondefault GMS
...
For the steps to specify these settings, see xref:#gkoac[To Preconfigure
Nondefault GMS Configuration Settings] and xref:#gkqqo[To Change GMS
...
information, see xref:#gkqdm[To Create a Cluster].
...
cluster settings during cluster creation as explained in xref:#gkqdm[To
Create a Cluster].
...
multicast not being configured properly. See xref:#gklhd[To Validate
...
* xref:#CHDCGIFF[To Discover a Cluster When Multiple Instances in a
Cluster are Running on a Host]
* xref:#CHDIGFCG[To Discover a Cluster When Each Instance in a Cluster
...
[NOTE]
====
For complete instructions for creating a cluster, see xref:#gkqdm[To Create a Cluster].
====
...
* xref:#gkqdm[To Create a Cluster]
* link:instances.html#gkqch[To Create an Instance Centrally]
* link:instances.html#gkqbl[To Create an Instance Locally]
...
* xref:#gkqdm[To Create a Cluster]
* xref:#gkqdn[To List All Clusters in a Domain]
* xref:#gkqcp[To Delete a Cluster]
...
For more information, see xref:#gkoac[To Preconfigure Nondefault GMS
Configuration Settings].
...
discovery-instances::
  ...
  see xref:#CHDGAIBJ[Discovering a Cluster When Multicast Transport Is
  Unavailable].
gms-port::
  The port number of the port on which the cluster listens for messages
  from GMS.
