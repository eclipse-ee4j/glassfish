...
* xref:#ghopc[Defining Statistics That Are to Be Monitored]
* xref:#ghplw[Updating the Monitorable Object Tree]
* xref:#ghrma[Dotted Names and REST URLs for an Add-On Component's Statistics]
* xref:#ghptj[Example of Adding Monitoring Capabilities]
...
* xref:#gkadk[Defining an Event Provider]
* xref:#ghprg[Sending an Event]
...
For more information, see xref:#ghprg[Sending an Event].
...
send events, see xref:#ghopc[Defining Statistics That Are to Be
Monitored].

Updating the monitorable object tree involves the following tasks:

* xref:#ghpni[Creating Event Listeners]
* xref:#ghptp[Representing a Component's Statistics in an Event Listener Class]
* xref:#ghpml[Subscribing to Events From Event Provider Classes]
* xref:#ghppo[Registering an Event Listener]
...
method. For more information, see xref:#ghpml[Subscribing to Events From
...
module-providername::
  ...
  the definition of the event provider. See xref:#ghovq[Defining an
  Event Provider by Writing a Java Class] and xref:#gkaec[Defining an
...
probe-provider-name::
  ...
  the event provider. See xref:#ghovq[Defining an Event Provider by
  Writing a Java Class] and xref:#gkaec[Defining an Event Provider by
  Writing an XML Fragment].
event-type::
  The type of the event. This type is defined in the event provider
  class. For more information, see xref:#ghosi[Defining Event Types in
...
listener-parent-node::
  ...
  For more information, see xref:#ghppo[Registering an Event Listener].
path-to-statistic::
  The path through the monitorable object tree from the node under which
  ...
  event listener. For more information, see xref:#ghppo[Registering an
  Event Listener].
statistic-name::
  ...
  the statistic. For more information, see xref:#ghptp[Representing a
...
xref:#ghovq[Defining an Event Provider by Writing a Java Class].
...
in xref:#ghppo[Registering an Event Listener]. The example shows the
...
* xref:#ghpni[Creating Event Listeners]. The example shows the code of
the `ModuleStatsTelemetry` listener class.
* xref:#ghptp[Representing a Component's Statistics in an Event Listener
Class]. The example shows the code for representing the
`countmbeancount` statistic.
* xref:#ghpml[Subscribing to Events From Event Provider Classes]. The
...
This example illustrates how to send an event as explained in xref:#ghprg[Sending an Event].
