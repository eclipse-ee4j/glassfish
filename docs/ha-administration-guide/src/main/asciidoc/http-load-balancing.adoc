...
* xref:#abdgx[Setting Up HTTP Load Balancing]
...
* xref:#abdgy[Prerequisites for Setting Up HTTP Load Balancing]
* xref:#gksdt[Configuring {productName} with Apache HTTP Server and `mod_jk`]
* xref:#CHDCCGDC[Configuring {productName} with Apache HTTP Server and `mod_proxy_ajp`]
* xref:#abdgz[HTTP Load Balancer Deployments]
...
described in xref:#CHDCCGDC[Configuring {productName} with Apache
...
* xref:#abdha[Using Clustered Server Instances]
* xref:#abdhc[Using Multiple Standalone Instances]
