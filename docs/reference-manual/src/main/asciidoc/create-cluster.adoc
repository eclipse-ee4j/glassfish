...
link:deploy.html#deploy[`deploy`(1)] subcommand
* Creating resources by using subcommands such as
link:create-jdbc-resource.html#create-jdbc-resource[`create-jdbc-resource`(1)]
...
`--properties`::
  ...
  `GMS_DISCOVERY_URI_LIST`;;
    ...
    ** port is the port number of the port on which the instance listens
    ...
    see "link:ha-administration-guide/clusters.html#discovering-a-cluster-when-multicast-transport-is-unavailable[Discovering a Cluster When Multicast Transport
...
"link:ha-administration-guide/clusters.html#discovering-a-cluster-when-multicast-transport-is-unavailable[Discovering a Cluster When Multicast Transport Is
