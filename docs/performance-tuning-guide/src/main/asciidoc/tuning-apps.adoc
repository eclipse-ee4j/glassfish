...
* xref:#abebf[Java Programming Guidelines]
* xref:#abebp[Java Server Page and Servlet Tuning]
* xref:#abebw[EJB Performance Tuning]
...
* xref:#abebg[Avoid Serialization and Deserialization]
* xref:#abebh[Use `StringBuilder` to Concatenate Strings]
* xref:#abebi[Assign null to Variables That Are No Longer Needed]
* xref:#abebj[Declare Methods as final Only If Necessary]
* xref:#abebk[Declare Constants as static final]
* xref:#abebl[Avoid Finalizers]
* xref:#abebm[Declare Method Arguments final]
* xref:#abebn[Synchronize Only When Necessary]
* xref:#abebo[Use DataHandlers for SOAP Attachments]
...
* xref:#abebr[General Guidelines]
* xref:#abebs[Avoid Shared Modified Class Variables]
* xref:#abebt[HTTP Session Handling]
* xref:#abebu[Configuration and Deployment Tips]
...
* xref:#abebx[Goals]
* xref:#abeby[Monitoring EJB Components]
* xref:#abeca[General Guidelines]
* xref:#abecg[Using Local and Remote Interfaces]
* xref:#abecj[Improving Performance of EJB Transactions]
* xref:#abecs[Using Special Techniques]
* xref:#abecv[Tuning Tips for Specific Types of EJB Components]
* xref:#abedd[JDBC and Database Access]
* xref:#abedi[Tuning Message-Driven Beans]
...
(CMRs). For more information, see xref:#abedc[Pre-Fetching Container
Managed Relationship (CMR) Beans].
* Identify read-only beans: Configure read-only entity beans for read
only operations. For more information, see xref:#abeda[Read-Only Entity
Beans].
...
* xref:#abeck[Use Container-Managed Transactions]
* xref:#abecl[Do Not Encompass User Input Time]
* xref:#abecm[Identify Non-Transactional Methods]
* xref:#abecn[Use `TX_REQUIRED` for Long Transaction Chains]
* xref:#abeco[Use Lowest Cost Database Locking]
* xref:#abecp[Use XA-Capable Data Sources Only When Needed]
* xref:#abecq[Configure JDBC Resources as One-Phase Commit Resources]
* xref:#abecr[Use the Least Expensive Transaction Attribute]
...
* xref:#abect[Version Consistency]
* xref:#abecu[Request Partitioning]
...
* xref:#abecw[Entity Beans]
* xref:#abecx[Stateful Session Beans]
* xref:#abecz[Stateless Session Beans]
* xref:#abeda[Read-Only Entity Beans]
* xref:#abedc[Pre-Fetching Container Managed Relationship (CMR) Beans]
...
* xref:#abede[Use JDBC Directly]
* xref:#abedf[Encapsulate Business Logic in Entity EJB Components]
* xref:#abedg[Close Connections]
* xref:#abedh[Minimize the Database Transaction Isolation Level]
